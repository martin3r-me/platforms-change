<div>
    <div x-show="!collapsed" class="px-3 pt-3 pb-2 border-b border-[var(--nx-text)] mb-2">
        <span class="text-[10px] uppercase tracking-widest text-[color:var(--nx-muted)] font-medium">Change</span>
    </div>

    <div x-show="!collapsed" class="px-2 mb-1">
        <a href="{{ route('change.projects.index') }}" wire:navigate class="flex items-center gap-2.5 px-3 py-1.5 rounded-md text-[13px] text-[color:var(--nx-faint)] hover:bg-[var(--nx-text)] hover:text-white transition-colors">
            @svg('heroicon-o-squares-2x2', 'w-4 h-4')
            <span>Projekte</span>
        </a>
        <a href="{{ route('change.kotter') }}" wire:navigate class="flex items-center gap-2.5 px-3 py-1.5 rounded-md text-[13px] text-[color:var(--nx-faint)] hover:bg-[var(--nx-text)] hover:text-white transition-colors">
    ...
    <div x-show="collapsed" class="px-2 py-2 border-b border-[var(--nx-text)]">
        <div class="flex flex-col gap-2">
            <a href="{{ route('change.projects.index') }}" wire:navigate class="flex items-center justify-center p-2 rounded-md text-[color:var(--nx-muted)] hover:text-white hover:bg-[var(--nx-text)] transition-colors" title="Projekte">
                @svg('heroicon-o-squares-2x2', 'w-5 h-5')
            </a>
            <a href="{{ route('change.kotter') }}" wire:navigate class="flex items-center justify-center p-2 rounded-md text-[color:var(--nx-muted)] hover:text-white hover:bg-[var(--nx-text)] transition-colors" title="Kotter Guide">
